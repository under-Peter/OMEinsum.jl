...
@noinline function dynamic_batched_contract(@nospecialize(iAs), @nospecialize(A), @nospecialize(iBs), @nospecialize(B), @nospecialize(iOuts))
    pA, iAps, iAbs, iAss, pB, iBps, iBbs, iBss, pOut = _analyse_batched_dim(iAs, iBs, iOuts)
    sAb, sAs, sAp, sBs, sBb, sBp, sAB = _analyse_batched_size(iAs, iAps, iAbs, iAss, collect(size(A)), iBs, iBps, iBbs, iBss, collect(size(B)))

    A, B = align_eltypes(A, B)
    pma = _conditioned_permutedims(A, pA, iAs)
    pmb = _conditioned_permutedims(B, pB, iBs)
    Apr = reshape(pma, sAb, sAs, sAp)
    Bpr = reshape(pmb, sBs, sBb, sBp)
    AB = _batched_gemm('N','N', Apr, Bpr)
    AB = _conditioned_permutedims(reshape(AB, sAB...), pOut, iOuts)
end

function _analyse_batched_dim(iAs, iBs, iOuts)
    iABs = iAs ∩ iBs
    pres   = iABs ∩ iOuts
    broad  = setdiff((iAs ∩ iOuts) ∪ (iBs ∩ iOuts), pres)
    summed = setdiff(iABs, pres)
...
function _analyse_batched_size(iAs, iAps, iAbs, iAss, sA, iBs, iBps, iBbs, iBss, sB)
    sAbs = getindex.(Ref(sA), indexpos.(Ref(iAs), iAbs))
    sAb = reduce(*, sAbs, init=1)
    sAs = mapreduce(i->sA[indexpos(iAs,i)], *, iAss, init=1)
    sAps = getindex.(Ref(sA), indexpos.(Ref(iAs),iAps))
